...
from database import get_db as get_db_connection_from_db_file, DATABASE_PATH, COPIAS_SEGURIDAD_FOLDER
...
def generar_pdf():
    try:
        ...
        #  Definir available_width después de que 'doc' y sus márgenes estén establecidos.
...
@export_bp.route('/get-available-backups', methods=['GET'])
def get_available_backups():
    try:
        # Asegúrate de que la carpeta de copias de seguridad exista
        if not os.path.exists(COPIAS_SEGURIDAD_FOLDER):
            os.makedirs(COPIAS_SEGURIDAD_FOLDER)  # La crea si no existe
            return jsonify([])  # No hay copias si la acabamos de crear

        # Lista los archivos .db en la carpeta de copias de seguridad
        db_files = [f for f in os.listdir(COPIAS_SEGURIDAD_FOLDER) if f.lower().endswith('.db')]

        # Ordena los archivos por fecha de modificación (los más recientes primero)
        db_files.sort(key=lambda f: os.path.getmtime(os.path.join(COPIAS_SEGURIDAD_FOLDER, f)), reverse=True)

        # Devuelve solo los nombres de archivo
        return jsonify(db_files)

    except Exception as e:
        current_app.logger.error(f"Error al obtener la lista de copias de seguridad disponibles: {e}", exc_info=True)
        return jsonify({"error": "No se pudo obtener la lista de copias de seguridad."}), 500


@export_bp.route('/crear-copia-seguridad', methods=['POST'])
def crear_copia_seguridad():
    try:
        db_original_path = DATABASE_PATH
        db_filename = os.path.basename(db_original_path)
        backup_dir = COPIAS_SEGURIDAD_FOLDER
        ...
        extension = os.path.splitext(db_filename)[1]
...
@export_bp.route('/restaurar-copia-seguridad', methods=['POST'])
def restaurar_copia_seguridad():
    try:
        ...
        if not selected_backup_filename:
            ...
            return "No se ha seleccionado ninguna copia de seguridad para restaurar.", 400

        if not secure_filename(selected_backup_filename) == selected_backup_filename:
            current_app.logger.warning(f"Intento de path traversal detectado: {selected_backup_filename}")

            return "Nombre de archivo no válido o seguro.", 400

        source_backup_path = os.path.join(COPIAS_SEGURIDAD_FOLDER, selected_backup_filename)
        target_db_path = DATABASE_PATH

        if not os.path.exists(source_backup_path):
            current_app.logger.error(
                f"Copia de seguridad '{selected_backup_filename}' no encontrada en: {COPIAS_SEGURIDAD_FOLDER}")
            # ### CAMBIO: Devuelve mensaje directo para JS ###
            return f"La copia de seguridad '{selected_backup_filename}' no fue encontrada.", 404

        # LÓGICA DE CIERRE DE CONEXIÓN Y REEMPLAZO DIRECTO (ALTO RIESGO DE BLOQUEO EN CALIENTE)
        db_connection_g = g.pop('db', None)
        if db_connection_g is not None:
            try:
                db_connection_g.close()
                current_app.logger.info("INFO: Conexión a la base de datos principal cerrada antes de la restauración.")
            except Exception as e:
                current_app.logger.warning(f"Advertencia: No se pudo cerrar la conexión g.db antes de restaurar: {e}",
                                           exc_info=True)
                # No lanzamos la excepción aquí, continuamos intentando la copia.

        # Copia el archivo de copia de seguridad sobre el archivo de base de datos principal.
        shutil.copy2(source_backup_path, target_db_path)
        current_app.logger.info(
            f"INFO: Base de datos restaurada directamente desde: {source_backup_path} a {target_db_path}")

        # Re-establece la conexión a la base de datos después de la restauración.
        g.db = get_db_connection_from_db_file()
        current_app.logger.info("INFO: Conexión a la base de datos principal re-establecida.")

        # Devuelve mensaje de éxito directo para JS ###
        return f"Copia de seguridad '{selected_backup_filename}' restaurada exitosamente.", 200

    except (sqlite3.OperationalError, PermissionError) as e:
        current_app.logger.error(f"ERROR: No se pudo restaurar la base de datos debido a bloqueo del archivo: {e}",
                                 exc_info=True)
        # Devuelve mensaje de error directo para JS ###
        return f"Error: La base de datos está en uso y no se pudo restaurar directamente. Por favor, Cierre y Reabra el programa e inténtelo de nuevo, o pruebe a reiniciar el PC si el problema persiste. Detalles: {e}", 500
    except Exception as e:
        current_app.logger.error(f"Error inesperado al restaurar la copia de seguridad: {e}", exc_info=True)
        # Devuelve mensaje de error directo para JS ###
        return f"Error inesperado al restaurar la copia de seguridad. Detalles: {e}", 500
